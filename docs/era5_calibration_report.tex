...
the ground in the $\pm12^\circ$ composite of Fig.~\ref{fig:vert} (369~hPa
averaged over $|\varphi|\le20^\circ$), a mass ratio of 2.4, so the model's own
geometry read literally
...
\section{The observed fluxes, mean against eddy}
\label{sec:fluxes}

The gross stabilities of Section~\ref{sec:stability} are properties of the mean
circulation alone. Fig.~\ref{fig:fluxes} puts that mean transport next to the
eddy transport it competes with, for moisture and for moist static energy, with
moisture converted to energy units by multiplying by $L_v$ so the two share an
axis.

Both splits use the same route. The mean part comes straight from the zonal
means, $(1/g)\int [v][x]\,dp$. The total comes from the column budget, whose
source is a stored surface or top-of-atmosphere field and so contains every
eddy the reanalysis has, stationary and transient. The eddy part is the
difference. Zonal averaging destroys the covariance that would let the eddy
flux be formed directly, which is why the budget route is needed at all. For
moisture the source is $E-P$; for energy it is the net downward flux at the top
of the atmosphere minus the net downward flux at the surface, radiative and
turbulent.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{era5_flux_decomposition.png}
\caption{Annual-mean column fluxes, positive northward, per unit length along a
latitude circle. (a) Latent flux $L_v F_q$: the mean circulation carries
moisture equatorward into the ITCZ in both hemispheres while the eddies carry
it poleward. (b) Moist static energy: the mean flux is small and the eddies
dominate poleward of about $20^\circ$. (c) The mean MSE flux decomposed. It is
the residual of a dry-static-energy flux and an opposing latent flux, each two
to three times larger than their sum.}
\label{fig:fluxes}
\end{figure}

Three things are worth reading off it.

\textbf{The mean MSE flux is a residual, and a fairly small one.} At
$15^\circ$N the mean circulation carries $+83.1$~MW\,m$^{-1}$ of dry static
energy poleward against $-49.0$~MW\,m$^{-1}$ of latent energy equatorward,
netting $+34.1$. At $10^\circ$N it is $+61.3$ against $-40.4$, netting $+20.9$.
That ratio, roughly $1 - W/\Wstar$, is the observational content of
$\Hhat/\Shat = 0.30$ from Section~\ref{sec:invariant}, seen in the flux rather
than in the coefficient.

\textbf{The sign of the mean MSE flux is poleward in the subtropics of both
hemispheres}, $+35.4$~MW\,m$^{-1}$ at $20^\circ$N and $-30.0$ at $20^\circ$S.
The mean circulation exports moist static energy out of the deep tropics, which
is the positive-gross-moist-stability behaviour, and it is what the model at
$a = 0.85$ and $W_c = 50$ failed to reproduce (Table~\ref{tab:acheck}, where
that run has the mean flux at $+104.7$~MW\,m$^{-1}$ against an eddy flux of
$106.2$ in near-cancellation).

\textbf{The eddies dominate outside the deep tropics.} The eddy MSE flux passes
the mean near $18^\circ$N and reaches $+96.6$~MW\,m$^{-1}$ at $30^\circ$N
against a mean of $+7.1$. The model has no representation of that transport for
energy: its only eddy term is the moisture diffusion $-D\,\py W$.

Two caveats. The moisture budget closes well, leaving $+4.2$ and
$-1.4$~MW\,m$^{-1}$ at $85^\circ$N and S where the flux must vanish, against a
tropical signal near $60$. The energy budget closes worse, leaving $+31.0$ and
$-27.1$~MW\,m$^{-1}$ against a peak near $140$, about 20\%. That is a
latitude-dependent bias in ERA5's energy budget which removing the global mean
cannot fix, so the eddy MSE curve is the least certain thing in this figure.
For scale, the total MSE flux at $15^\circ$N is $51.9$~MW\,m$^{-1}$, which
multiplied around the latitude circle is 2.01~PW, a reasonable atmospheric
...
7.4~mm\,day$^{-1}$, the jet weakens from 73.8 to 58.7~m\,s$^{-1}$ while its
deepest easterly notch shallows from $-45.5$ to $-16.6$~m\,s$^{-1}$ and moves
poleward from 7.48 to 9.53~Mm, and the mean MSE flux returns to a modest
poleward
...
\subsection*{The model's own fluxes, against the observed ones}

Fig.~\ref{fig:modelfluxes} is the model's counterpart of Fig.~\ref{fig:fluxes},
in the same units and sign convention, with a second axis giving the latitude
each $y$ maps to through $f = \beta y$. The model has exactly one eddy term,
the moisture diffusion $-L_v D\,\py W$, so its eddy latent flux and its eddy
MSE flux are the same curve and nothing in it transports dry static energy
except the mean circulation.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{era5_a_check_fluxes.png}
\caption{The model's column fluxes for the three runs, sharing a vertical scale
within each row so the aggregated run's much larger fluxes read as larger. Top:
latent flux, mean and eddy. Bottom: the MSE flux with the mean decomposed into
its dry-static and latent parts. The eddy curve (orange) is close to zero
everywhere in all three runs.}
\label{fig:modelfluxes}
\end{figure}

\begin{table}[htbp]
\centering
\caption{Column fluxes at $24^\circ$, which is $y = 3$~Mm under the Coriolis
mapping, in MW\,m$^{-1}$ positive northward. ERA5 is the 2000--2009
annual mean.}
\label{tab:fluxcompare}
\begin{tabular}{lccccc}
\toprule
 & DSE mean & $L_v q$ mean & MSE mean & eddy & total\\
\midrule
ERA5                   & $+50.7$  & $-22.9$  & $+27.9$ & $+57.7$ & $+85.6$\\
$a=0.85$, $W_c=50$     & $+151.2$ & $-148.9$ & $+2.3$  & $-2.2$  & $+0.1$\\
$a=0.77$, $W_c=50$     & $+114.5$ & $-100.5$ & $+14.0$ & $+0.5$  & $+14.5$\\
$a=0.85$, $W_c=40$     & $+124.2$ & $-113.0$ & $+11.2$ & $+0.5$  & $+11.7$\\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:fluxcompare} is the sharpest model-observation comparison in this
report, and it is not flattering. Taking the recommended run at $a = 0.77$:

\begin{itemize}
\item Its mean dry-static-energy flux is $+114.5$ against an observed $+50.7$,
2.3 times too large, and its mean latent flux is $-100.5$ against $-22.9$,
4.4 times too large.
\item Those two nearly cancel, so its mean MSE flux is $+14.0$ against an
observed $+27.9$, about half. The model both over-transports each component and
under-transports their difference.
\item Its eddy flux is $+0.5$ against an observed $+57.7$. At this latitude the
observed eddies carry twice what the mean circulation does, and the model has
essentially none of it.
\item So its total column energy transport across $24^\circ$ is
$+14.5$~MW\,m$^{-1}$ against an observed $+85.6$, a factor of six.
\end{itemize}

The missing eddy energy transport is structural rather than a matter of
calibration. The eddy flux is linear in $D$, so closing the gap from $+0.5$ to
$+57.7$~MW\,m$^{-1}$ at fixed $\py W$ would take
$D = 1.2\times10^{8}$~m$^2$\,s$^{-1}$, against the $3\times10^{5}$ to
$4\times10^{6}$ that Section~\ref{sec:other} measures. The reason is that the
model's $W$ is nearly flat, pinned near $\Wq$ by the local
evaporation--precipitation balance, so there is almost no gradient for $D$ to
act on. And an eddy dry-static-energy flux is not in the equations at all.
This is the largest gap the calibration exposes.
...
overshoot is shared by every V2 configuration and predates this calibration; the
2026-07-30 attribution chain traced it to the radiative retarget steepening the
relaxation target, a comparison not re-run for this report. It is a separate
...
\textbf{The model transports about a sixth of the observed column energy, and
has no eddy energy flux.} Table~\ref{tab:fluxcompare}: at $24^\circ$ the
recommended run carries $+14.5$~MW\,m$^{-1}$ against an observed $+85.6$,
because its eddy flux is $+0.5$ against $+57.7$ and its mean flux is the small
residual of a dry-static and a latent part each two to four times too large.
This is the largest model-observation discrepancy in the report and the one
least amenable to calibration, since the model has no eddy dry-static-energy
term and its $W$ is too flat for the moisture diffusion to act on. Whether V2
is meant to represent that transport at all is a question about the model's
scope, not about its parameters.
...
Transcribed from \path|ss09/sw_model.py| (\path|du_dt|, \path|dv_dt|,
\path|dtheta_dt|, \path|moisture_transport_tendency|) and cross-checked against
\path|SCIENCE.md| \S2.1--2.3 and \S3.1. $\theta_{\rm rad}$ is the radiative
relaxation target, $\Lambda = L_v/C$, $T = \Pi\theta$ with $\Pi = 1/1.6$, and
$\mathcal{H}$ is the Heaviside step with $\mathcal{H}(0) = 1/2$:
%
\begin{align}
\partial_t u &= \beta y v - v\,\py u - \mathcal{H}(G)\,(\py v)\,u
  - \epsilon_u u - v_d\,\mathcal{H}(u)\,\mathrm{sgn}(y)\,\py u,
\label{eq:u}\\
2\,\partial_t v &= -\beta y u - \frac{gH}{T_0}\,\py T
  + \kappa_v \partial_y^2 v,
\label{eq:v}\\
\partial_t \theta + \frac{\delta\,\Delta_z}{H}\,\py v
  &= \frac{\theta_{\rm rad} - \theta}{\tau} + \kappa_\theta \partial_y^2\theta
  + \Lambda P,
\label{eq:th}\\
...
Four points where these differ from what a reader might expect, each of which I
had wrong in the first version of this report:

\begin{itemize}
\item The eddy momentum flux divergence in~\eqref{eq:u} is an \emph{advection}
at the poleward velocity $v_d\,\mathrm{sgn}(y)$, gated on westerlies. It is not
the divergence of a flux $v_d \mathcal{H}(u) u$. Because
$\mathcal{S} \propto \mathrm{sgn}(y)\,\py u$ it is up-gradient at a wind
maximum, which is what makes the jet flanks the delicate region of the model.
\item The pressure-gradient term in~\eqref{eq:v} is written directly in
temperature, $-(gH/T_0)\py T$, with $T_0 = 300$~K a fixed reference. There is
no geopotential in the model. Using $\theta$ in place of $T$ here was one of
the two errors fixed by the SS13 corrigendum.
\item The vertical-advection gate $\mathcal{H}(G)$ sits on the \emph{momentum}
equation only. $G = \theta_E - \theta$ for dry and V1 runs, $G = \py v$ under
the \path|kinematic| gate that V2 uses. The thermodynamic
equation~\eqref{eq:th} is ungated.
\item $\delta$ is the model's layer depth, called $d$ in the body of this
report and \path|delta| in the code. $\kappa_\theta$ is zero by default.
\end{itemize}
...
Parameter values used above: $\delta = 4$~km, $\Delta_z = 60$~K, $H = 16$~km,
$T_0 = 300$~K, $g = 9.81$~m\,s$^{-2}$,
...
One inconsistency to be aware of when reading numbers across this report: the
model uses $g = 9.81$ while the ERA5 scripts use $g = 9.80665$, so $\Shat$ and
$\Wstar$ computed on the two sides differ in their fourth digit. That is why
Table~\ref{tab:invariant} gives $\Wstar = 44.3$~kg\,m$^{-2}$ from the ERA5-side
calculation and Table~\ref{tab:acheck} gives 44.2 from the model runs' own
attributes. It changes nothing.
...
\path|era5_moisture_budget.py| & Section~\ref{sec:other};
  Fig.~\ref{fig:budget}\\
\path|era5_flux_decomposition.py| & Section~\ref{sec:fluxes};
  Fig.~\ref{fig:fluxes}\\
