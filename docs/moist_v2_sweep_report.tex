...
The precipitation rates are far outside anything observed. Against the model's own
uniform source of 3.97~\mmday{} the equatorial spike at $W_c = 50$ is 19.6 times
larger, and the $D = 0$ peak of 831.7 is 209 times larger. In observational terms
a zonal-mean 77.8~\mmday{} is roughly an order of magnitude above the wettest
zonal-mean tropical band, which sits near 6 to 8~\mmday, though I have not
checked that figure against data in this session. This is the clearest sense in
which the negative-stability solutions are not candidate Hadley circulations.
...
\begin{center}
\input{tables/seasonal}
\end{center}

\noindent The last two columns are the largest jet reached in each hemisphere
over the cycle, and they are equal for every positive-stability run because a
symmetric migration visits each hemisphere's solstice in turn. They differ only
for the chaotic runs, where the two solstices of one arbitrary cycle are not
mirror images.
