...
The `scripts/era5_*` modules calibrate model parameters against ERA5 monthly zonal means (data at `~/Dropbox/data/ecmwf/era5/monthly/`, symlinked as `era5-data/`): `era5_a_calibration.py` (+`_figures.py`) measures the lower-branch CWV fraction a, `era5_stability_calibration.py` the gross dry and gross moist stabilities, `era5_vertical_structure.py` the branch geometry and the barotropic-mass-adjustment sensitivity, and `era5_moisture_budget.py` the eddy diffusivity D and the RCE temperature contrast. **Read `docs/era5_calibration_notes.md` before using any of their numbers**: the model mixes two incompatible normalizations of v (the moisture equation's `-(2a-1)vW` needs branches holding half the column mass each, while the thermodynamic equation's `d Delta_z/H` describes a layer of depth d), every transport coefficient scales linearly between them by `dp/(p_s/2)`, and that mismatch is the whole negative-gross-moist-stability problem rather than a physics defect. The existing figures were all made under the half-column reading and must be regenerated before reuse.
