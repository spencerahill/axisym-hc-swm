...
See **SCIENCE.md** for detailed physics background (governing equations, parameterizations, dynamical regimes). See **docs/fixed_ro_beta_plane_theory.pdf** (source: the `.tex` beside it; rebuild with `latexmk -pdf -output-directory=docs docs/fixed_ro_beta_plane_theory.tex`) for the fixed-Ro suite's beta-plane theory memo and its run record. See **docs/moist_v1_sensitivity_report.pdf** for the self-contained moist V1 status report (the four parameter scans, the gross-moist-stability regime boundary they map, full model equations and parameter values, run inventory; rebuild with `latexmk -pdf -output-directory=docs docs/moist_v1_sensitivity_report.tex`, figures regenerated by the `scripts/moist_v1_*` analysis scripts). See **docs/moist_v1_mse_budget.pdf** for the moist V1 gross-dry/moist-stability and MSE-flux-decomposition derivation with the first equilibrium numbers (`scripts/moist_v1_analysis.py` regenerates the figures from a D-ladder run directory). `scripts/moist_v1_wc_scan_analysis.py` analyzes the companion W_c scan (35-55 at fixed D=1e6, runs in `model_output/moist_v1_wc_scan/`) across the gross-moist-stability crossover at W_c* = 43.9. `scripts/moist_v2_analysis.py` analyzes the moist V2 equilibria in `model_output/moist_v2/` (the V1-to-V2 attribution chain, the Delta_theta^rad ladder, and the gross-moist-stability regime comparison across W_c); it builds the MSE fluxes on the faces with the model's own operators, which a center reconstruction gets wrong by order the source itself. `scripts/moist_v1_tauc_dz_analysis.py` analyzes the tau_c (1800-172800 s) and Delta_z (45-90 K) sensitivity scans (runs in `model_output/moist_v1_tauc_dz/`): tau_c sets the quiescent collar W_c + tau_c E_0 and the ~linear-in-tau_c transport departures from it, while Delta_z moves both the dry circulation and the crossover W* itself, with Hhat's sign flipping at the predicted Delta_z* = 68.2 K at default moist parameters.

The `scripts/era5_*` modules calibrate model parameters against ERA5 monthly zonal means (data at `~/Dropbox/data/ecmwf/era5/monthly/`, symlinked as `era5-data/`). **See `docs/era5_calibration_report.pdf` for the full result**; rebuild with `latexmk -pdf -output-directory=docs docs/era5_calibration_report.tex` from the repo root. `era5_normalization.py` is the one to read first: the model mixes two incompatible normalizations of v (the moisture equation's `-(2a-1)vW` needs branches holding half the column mass each, while the thermodynamic equation's `d Delta_z/H` describes a layer of depth d), every transport coefficient scales linearly between them by `dp/(p_s/2)`, and that mismatch was the whole negative-gross-moist-stability problem rather than a physics defect. `W* = Shat/(L_v(2a-1))` and `Hhat/Shat` are invariant under the choice and are what to compare with observations; ERA5 gives `W* = 44-57` kg/m^2 against 44.3 for the model at `a = 0.85`, so the default sits at the extreme strong-transport end and is the one choice that puts a `W_c = 50` run below its own crossover. Recommended `a ~ 0.79` (range 0.77-0.84). The rest: `era5_a_calibration.py` (+`_figures.py`) measures the lower-branch CWV fraction a as a mass partition, `era5_stability_calibration.py` the gross dry and gross moist stabilities, `era5_vertical_structure.py` the branch geometry and the barotropic-mass-adjustment sensitivity (a 34% floor on the Shat uncertainty), `era5_moisture_budget.py` the eddy diffusivity D, E_0 and the RCE temperature contrast, `era5_flux_decomposition.py` the column moisture and MSE fluxes split into mean-circulation and eddy parts (moisture in energy units; the eddy part comes from the column budget, since zonal averaging has destroyed the covariance), and `era5_calibration_v2_check.py` the model test confirming that the ERA5-consistent `a` removes the aggregated V2 regime. **The report's Appendix A is the authoritative transcription of the model equations**; `SCIENCE.md` §2-3 is the other correct source. Do not write the equations from memory or from the spec PDF: an earlier draft of the report had the EMFD as a flux divergence rather than `v_d H(u) sgn(y) du/dy`, put a geopotential in the v equation instead of `gH/T_0 dT/dy`, and gated the thermodynamic equation, which is ungated.

## Calibration diagnostics come in pairs

Any observational diagnostic built to calibrate or evaluate this model gets its model counterpart produced in the same pass, in the same units, the same sign convention, and on a comparable axis (use the Coriolis mapping `f = beta y = 2 Omega sin(phi)` to turn model `y` into a latitude, which is what `scripts/era5_normalization.py` recommends and `era5_calibration_v2_check.py` implements as a secondary axis). The point of the ERA5 work is a model-observation comparison, so an observational curve with no model curve beside it is half a diagnostic.

**Why:** on 2026-08-01 the ERA5 mean-vs-eddy flux decomposition was built for the observations only, and Spencer had to ask for the model's. Producing it took twenty minutes and exposed the largest finding of the session: at 24 degrees the model carries +14.5 MW/m of column energy against an observed +85.6, because its eddy flux is +0.5 against +57.7 and its mean flux is the small residual of a dry-static and a latent part each two to four times too large. That would not have surfaced from the observational panel alone.
...
**moist_constants (`moist_constants.py`)**: the column thermodynamic constants shared by the model and the analysis scripts. `column_heat_capacity(g)` = c_p Pi Delta_p/g = 5.16e6 J/m^2/K, `latent_heating_coeff(g)` = L_v/C = 0.4843 K per kg m^-2 s^-1, `gross_dry_stability(...)` = C d Delta_z/H, plus L_V and the Exner/pressure constants they derive from. Import these rather than re-deriving: C*Lambda = L_v is what makes precipitation cancel in the MSE budget, and the older V1 analysis scripts hard-code a rounded C = 5.2e6 that breaks the cancellation at the 0.7% level
...
**Constraints** (hard errors in `SWConfig`): moist parameters without `--enable-moisture`; `--grid collocated` (the frozen Zhang et al. 2025 reproduction path). `--backend numba` is supported for moist runs (the W step is mirrored into the kernel, bitwise-identical; see the Integration Backends section).
...
**Regime warning: the V1-inherited default `--w-crit 50` puts V2 in the negative-gross-moist-stability corner, and the solution there is not a Hadley circulation** (measured 2026-07-31, ny=801, all runs in `model_output/moist_v2/`). Hhat = Shat - L_v(2a-1)W crosses zero at W = 44.25 kg/m^2 while the quiescent column sits at W_c + tau_c E_0, so W_c = 50 makes Hhat negative over 84% of the domain. The mean circulation then imports MSE into the ascending column instead of exporting it, ascent feeds itself, and the only brake is the eddy flux, which needs a steep moisture gradient. The equilibrium aggregates: a 78 mm/day equatorial spike 394 km wide (peak precipitation is exactly (W_max - W_c)/tau_c), a second precipitating band at +/-5.6 Mm at 18.7 mm/day, a bone-dry gap where P is identically zero over 36% of the domain, and an extra easterly notch carved into the jet at 5.59 Mm with the terminus notch dragged equatorward to 7.48 Mm and deepened to -45 m/s. It is resolution-converged (ny=1601 reproduces the secondary band at 5.61 Mm, same 18.7 mm/day), so it is a real solution of these equations rather than grid noise. One step across the crossover at `--w-crit 40` restores a recognizable Hadley cell: one broad ITCZ at 9.0 mm/day, one jet, one notch, W nearly flat, and the mean MSE flux back to +14.5 MW/m poleward with the eddy flux a 1.5 MW/m bit player (against -104.7 and 106.2 MW/m at W_c = 50). Spencer's call 2026-07-31: the V2 default must be a positive-GMS one; the aggregated regime is real but not of current interest, with slightly negative GMS the possible exception. **Resolved 2026-08-01 by the ERA5 calibration**: the 44.25 crossover is an artifact of the spec's `a = 0.85`, which is at or just beyond the strong-transport end of what ERA5 supports. Lowering `a` raises the crossover instead of lowering the column, and is the better fix because it moves the model toward the observations rather than away. `--cwv-frac 0.77` puts W* at 57.4, above the quiescent 50.66, and `model_output/moist_v2/wc50_a077/` confirms it: at `W_c = 50` the aggregated state does not appear and the run reproduces the `W_c = 40` control in every field. Recommended `a ~ 0.79` with range 0.77-0.84 (`docs/era5_calibration_report.pdf` section 7); regenerate the comparison with `python scripts/era5_calibration_v2_check.py`.
...
### The V2 parameter sweep (151 runs, 2026-08-09)

**See `docs/moist_v2_sweep_report.pdf` for the full result.** `scripts/v2_sweep_manifest.py` is the single source of truth for what was run and is the file to read first; `v2_sweep_run.py` drives a process pool and has a `--smoke N` stability screen; `v2_sweep_diagnostics.py` builds the scalar table with the model's own face operators; `v2_sweep_figures.py` and `v2_sweep_report_tables.py` produce every figure and every number the report quotes; `v2_sweep_verify.py` re-checks the two configurations run in July against what this file and the ERA5 report record for them (all 18 quantities reproduce, most under 1%). Output in `model_output/moist_v2_sweep/`, 33 GB, with `all_runs.csv` and `numbers.json` beside it.

**One dimensionless number organises most of V2, and it is worth computing before choosing parameters.** `r = W_q/W* = L_v(2a-1)(W_c + tau_c E_0)/(C d Delta_z/H)`, the quiescent column over the Hhat = 0 crossover. Five parameters move it and four of them collapse onto it: eight runs at matched r with `a` from 0.75 to 0.87 and W_c from 34.9 to 52.0 agree on the jet to 0.19% and peak |v| to 0.10% at r = 0.85, against sweep-wide spreads of 78% and 382%. `tau_c` and `E_0` do NOT collapse, because each also sets something else (how fast convection removes water; how much water there is): the tau_c ladder crosses r = 1 with the domain still raining everywhere, since a slow convective sink smears rain out and concentrating it is what aggregation requires.

**The regime boundary is at r ~ 0.94, not at r = 1**, because convergence lifts W above W_q in the ascending region so the local Hhat goes negative before the undisturbed column's does. Measured on the a = 0.85 transect: raining everywhere at r = 0.919, three bands over 91% of the domain at r = 0.964. **A second boundary near r = 1.3 is where solutions stop settling at all**: 18 of 131 never reach a steady state (jet swinging 2.6-83.0 m/s, or bands wandering while the jet holds), and the largest r that still settles is 1.310. Classify with `steady` in `all_runs.csv`; the MSE budget residual confirms it independently (3e-9 to 6e-4 for the 113 steady runs, 0.12 to 1690 for the rest, since the budget omits d<h>/dt).

**The eddy moisture flux D is the brake, and only on the negative side.** At a = 0.85, W_c = 50 the mean circulation carries +104.7 MW/m into the ascending column and the eddy flux carries -106.2 out, cancelling to 1.4%. So D has more leverage over that state than any other parameter and is the one ERA5 constrains least (3e5 to 4e6 gives raining fractions 0.54 to 0.72); at D = 0 the solution collapses to 4.6% of the domain raining at 832 mm/day. On the positive side the same range of D changes peak P by 3%, so **D cannot be calibrated from a positive-GMS run**.

**Aggregation needs Lambda at nearly full strength** (nothing at 0, 0.25, 0.50, 0.75 of L_v/C; peak P jumps 7.9 -> 77.8 mm/day between 0.75 and 1.0), and **with passive moisture V1 crossing r = 1 does nothing at all** (two V1 twins at r = 0.76 and 1.14 have identical circulations while Hhat is +18.4 and -11.4 MJ/m^2). Aggregation survives v_d = 0, so it is thermodynamic rather than eddy-driven.

**dt = 30 s at ny = 801 is the ceiling only at the reference parameters.** 22 of 149 configurations diverged within 100 days of a cold start; 16 need 15 s and 5 need 6 s. The pattern is a strong circulation (deeply negative Hhat, steep Delta_theta^rad, small Delta_z) or a forcing maximum at |y_0| >= 700 km. These are cold-start transients, not unstable equilibria: a seasonal run whose forcing WALKS out to y_0 = 1400 km survives at dt = 30 where one started there does not. The measured requirements are recorded as `DT_15` and `DT_6` in the manifest. **Always run the `--smoke 100` screen before a long campaign**; it cost 4 minutes and saved 22 wasted multi-hour runs.

**Two results to know before interpreting off-equatorial or seasonal V2 runs.** The rain maximum and the energy flux equator are 1.9 Mm apart at y_0 = 1400 km, because a nearly uniform Hhat makes the total flux essentially v*Hhat, which vanishes at the cell boundary rather than at the ascent maximum; there is no v zero between -5.0 and +3.0 Mm, so one cross-equatorial cell spans the tropics with the rain inside it. And the seasonal response is quasi-static: measured lag 3.1-6.0 days across periods of 90 to 1440 days against 17-37 days for a 37-day relaxation, with the gain matching the steady off-equatorial value at the same displacement. Every seasonal run on the negative-GMS reference is chaotic (consecutive cycles differ by 10,889-14,656 km in band position), so their fitted amplitudes and lags are meaningless; `seasonal_scalars` carries a `cyclic` flag for this.

**Diagnostic traps this sweep hit, all of which give plausible wrong answers.** The precipitation centroid over the whole domain is diluted by the drizzle covering it, reading 0.299 Mm for a band at 1.082 Mm; use `precip_peak` or `precip_excess_centroid` (the latter restricted to the connected excess region, or a second lobe makes the seasonal centroid LEAD the forcing by 38 days). The Rossby number averaged out from the rain centroid reaches into the equator where beta*y vanishes and reports 9.7; use the fixed |y| = 1-4 Mm band. A seasonal lag from a circular cross-correlation peak aliases on a series with a spin-up trend; fit cos/sin/mean/trend, and verify the sign against a synthetic signal. An "ascending branch" defined as the v zero turning poleward picks a subtropical feature 3-4 Mm from the rain; use the maximum of dv/dy.
...
- `test_moist_v2.py`: Moist V2 (column constants, config/CLI validation, the radiative retarget, both vertical-advection gates, the discrete MSE-budget closure, the Lambda=0 dry-twin identity, parity and restart under coupling)
